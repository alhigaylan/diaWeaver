...
init_hits.hits_.erase(it_zero, init_hits.hits_.end());
  ...
  FragmentIndex::FragmentIndex() : DefaultParamHandler("FragmentIndex")
  {
    ...
    defaults_.setValue("peptide:enzyme_specificity", "full",
      ...
      "  'semi-only' : exactly one terminus is enzyme-specific; fully-specific products are excluded.\n"
      "  'none'      : no enzyme constraint; every substring of length [min_size, max_size] is\n"
      "                enumerated. Canonical setting for immunopeptidomics (e.g. HLA 8..12mers).\n"
      "  'none-only' : neither terminus is enzyme-specific; semi- and fully-specific products are\n"
      "                excluded. Useful as a third search tier after 'full' and 'semi-only'.");
    defaults_.setValidStrings("peptide:enzyme_specificity", {"full", "semi", "semi-only", "none", "none-only"});
