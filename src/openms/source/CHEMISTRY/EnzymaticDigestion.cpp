...
namespace OpenMS
{
  const std::string EnzymaticDigestion::NamesOfSpecificity[] = {"none", "semi", "full", "unknown", "semi-only", "none-only", "unknown", "unknown", "no-cterm", "no-nterm"};
  ...
  Size EnzymaticDigestion::digestUnmodified(const StringView& sequence, std::vector<StringView>& output, Size min_length, Size max_length) const
  {
    ...
    // SPEC_NONE_EXCLUSIVE: all substrings where neither terminus is at a cleavage site.
    ...
    if (specificity_ == SPEC_NONE_EXCLUSIVE)
    {
      ...
      fp.push_back(static_cast<int>(sequence.size())); // fp is now sorted: [0, cut1, cut2, ..., seq.size()]
    ...
    // SPEC_SEMI appends these on top of fully-specific products above.
  ...
  Size EnzymaticDigestion::digestUnmodified(const StringView& sequence, std::vector<std::pair<Size, Size>>& output, Size min_length, Size max_length) const
  {
    ...
    if (specificity_ == SPEC_NONE_EXCLUSIVE)
    {
      ...
      for (Size i = 1; i + min_length <= sequence.size(); ++i)
      {
        ...
        {
          if (!std::binary_search(fp.begin(), fp.end(), static_cast<int>(j)))
            output.emplace_back(i, j - i); // (start, length)
        }
      }
      return 0;
    }
    ...
    if (specificity_ == SPEC_SEMI || specificity_ == SPEC_SEMI_EXCLUSIVE)
    {
      ...
      for (const auto& p : semi_pairs)
      {
        output.emplace_back(p.first, p.second - p.first); // convert (start, end) → (start, length)
      }
    }
